...
public class UserProvider extends ContentProvider {
    ...
    private static final String CONTENT_TYPE = "vnd.android.cursor.dir/vnd.proformatique.xivo.user";
    private static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.proformatique.xivo.user";
    ...
    private static final String DATABASE_CREATE = "create table " + DATABASE_TABLE + " (" + _ID
            + " integer primary key autoincrement, " + ASTID + " text not null, " + XIVO_USERID + " text not null, "
            + FULLNAME + " text not null, " + PHONENUM + " text not null, " + LINEID + " text, " + STATEID
            + " text not null, " + STATEID_LONGNAME + " text not null, " + STATEID_COLOR + " text not null, "
            + TECHLIST + " text not null," + HINTSTATUS_COLOR + " text not null, " + HINTSTATUS_CODE
            + " text not null, " + HINTSTATUS_LONGNAME + " text not null" + ");";
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        Cursor c = sqlBuilder.query(xivouserDB, projection, selection, selectionArgs, null, null, sortOrder);
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        case USERS:
            ...
            break;
        case USER_ID:
            count = xivouserDB.update(DATABASE_TABLE, values,
    ...
    public static long getIndex(Context context, String astid, String userid) {
        long index = -1;
        Cursor row = context.getContentResolver().query(CONTENT_URI, new String[] { _ID, ASTID, XIVO_USERID },
                ASTID + " = '" + astid + "' AND " + XIVO_USERID + " = '" + userid + "'", null, null);
        if (row.getCount() > 0) {
            row.moveToFirst();
    ...
    public static void updatePresence(Context context, long id, String stateId) {
        ...
        context.getContentResolver().update(CONTENT_URI, values, _ID + " = '" + id + "'", null);
    }
    ...
    public static void showUserInfo(Context context, String astid, String xivoid) {
        Cursor user = context.getContentResolver().query(CONTENT_URI, null,
                ASTID + " = '" + astid + "' and " + XIVO_USERID + " = '" + xivoid + "'", null, null);
        if (user.getCount() > 0) {
            user.moveToFirst();
            do {
                showUserInfo(user);
     ...
     * @param xivoid
     */
    public static long getDbUserId(Context context, String xivoid) {
        Cursor user = context.getContentResolver().query(UserProvider.CONTENT_URI,
                new String[] { UserProvider._ID, UserProvider.XIVO_USERID },
                UserProvider.XIVO_USERID + " = '" + xivoid + "'", null, null);
        if (user.getCount() > 0) {
            user.moveToFirst();
    ...
    public static long getDbUserIdFromLineId(Context context, String lineId) {
        Cursor user = context.getContentResolver().query(UserProvider.CONTENT_URI,
                new String[] { UserProvider._ID, UserProvider.LINEID }, UserProvider.LINEID + " = '" + lineId + "'",
                null, null);
        if (user.getCount() > 0) {
            user.moveToFirst();
            long id = user.getLong(user.getColumnIndex(UserProvider._ID));
    ...
    public static long getXiVOUserIdFromLineId(Context context, String lineId) {
        Cursor user = context.getContentResolver().query(UserProvider.CONTENT_URI,
                new String[] { UserProvider.XIVO_USERID, UserProvider.LINEID }, UserProvider.LINEID + " = '" + lineId + "'",
                null, null);
        if (user.getCount() > 0) {
            user.moveToFirst();
            long id = user.getLong(user.getColumnIndex(UserProvider.XIVO_USERID));
            user.close();
            return id;
        } else {
            user.close();
